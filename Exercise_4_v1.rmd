...
5. Produce a neatly formatted table of the regression in question 4.
...
7. Use your regression in question 3 to predict how many votes on average an indigenous man would receive in an average election for prefeito. (Remember to make both variables factors before you run the regression).
...
8. Create a density plot (`geom_density`) of the full set of predictions from question 7. 
...
data %>% filter(NUM_TURNO==1) %>% 
  ...
  zelig_qi_to_df() %>%
  ggplot() +
  geom_density(aes(x=predicted_value)) +
  theme_classic()
```

9. How does the number of votes received change for an indigenous woman?
