...
1. Create a new Rmarkdown document in Rstudio. Load the `tidyverse`,`zeligverse`, `knitr`, `cepespR`, `sf` and `mapview` packages.
```{r, echo=TRUE}
library(tidyverse)
library(zeligverse)
library(knitr)
...
6. Let's create a new shapefile with a different projection for the centroids data. Using `st_transform` we can specify a numerical code for an appropriate projection. Search on [this](http://spatialreference.org/ref/epsg/) site for Brazil and use the appropriate numeric code.

```{r}
centroids2 <- centroids %>% st_transform(29101)
```

7. For each of the two projections for the centroids, calculate the distance between the centroid of each municipality and the state capital (`CUIABÁ`). What is the distance between the capital and the municipality that is furthest away? Compare these two distances for both projections.
