...
#Sandwich_zelig
  ...
  # Minor difference in bread but not so bad
...
sigma <- sqrt(sum(model1$get_residuals()[[1]]^2)/(dim(estd)[1]-4))
(model$get_vcov()[[1]]/sigma^2)/summary(x)$cov.unscaled

#So in sigma the df adjustment is n+beta-beta; in the vcov matrix will multiply it back by n+beta
(model$get_vcov()[[1]]/sigma^2)*dim(estd)[1]

sum(summary(x)$df[1:2])
