...
data <- cepespdata(year=2014, position="Governor", regional_aggregation="Municipality",political_aggregation="Candidate",state="AC")
data %>% filter(NUM_TURNO==2)
table(data$NUM_TURNO)

data <- data %>% filter(NUM_TURNO==2) %>%
  group_by(COD_MUN_IBGE) %>%
  mutate(Vote_Pct=(QTDE_VOTOS/sum(QTDE_VOTOS))*100) %>%
  ungroup() %>%
  filter(NOME_URNA_CANDIDATO=="TIÃO VIANA")
...
#Cartograms - area of polygon proportional to variable
#devtools::install_github("sjewo/cartogram")
library(cartogram)

#  %>% st_transform("+proj=utm +datum=WGS84 +zone=45")
sf_cart <- sf_merged  %>% cartogram("Vote_Pct", itermax=5)

sf_cart %>% ggplot() + geom_sf(alpha=0.5) + theme_classic()

#Equal Area Cartograms - Hexmaps
devtools::install_github("jbaileyh/geogrid")
library(geogrid)

#Can do again direct with sf, at least for assign_polygons
sf_hex_grid <- sf_merged %>% as("Spatial") %>% calculate_grid(grid_type="hexagonal")
sf_hex_grid <- sf_merged %>% as("Spatial") %>% assign_polygons(sf_hex_grid)
